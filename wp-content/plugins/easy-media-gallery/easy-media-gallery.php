...
if ( !defined( 'EASYMEDIA_VERSION' ) ) {
	define( 'EASYMEDIA_VERSION', '1.3.117' );
}
...
|--------------------------------------------------------------------------
*/
if( !function_exists( "emg_plugin_update_changelogs" ) ){
	
	function emg_plugin_update_changelogs( $args ){
		
		global $pagenow;
		
		if ( 'plugins.php' === $pagenow ){
			
		wp_enqueue_style( 'easmedia_update_styles', plugins_url('includes/css/update.css' , __FILE__ ) );
		
		$response = wp_remote_get( 'https://plugins.svn.wordpress.org/easy-media-gallery/trunk/readme.txt' );
		...
		if ( ! is_wp_error( $response ) && ! empty( $response['body'] ) ) {
			...
			if ( preg_match( $regexp, $response['body'], $matches ) ) {
				...
				$upgrade_notice .= '<p class="emg_update_changelogs_ttl">Changelog:</p>';
				$upgrade_notice .= '<div class="emg_update_changelogs">';
				foreach ( $changelog as $index => $line ) {
					...
					if ( strpos( $line, '=' ) !== false ) {
						...
						$upgrade_notice .= "<p class='emg_version_clttl'>".$line."</p>";
